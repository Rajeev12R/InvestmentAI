...
const testSuites = [
  { name: 'Suite 1: Performance & Skill Types, Taxonomy & Schemas', file: 'test-performance-types-schema.js', category: 'Functional' },
  { name: 'Suite 2: Multi-Tenant & Point-in-Time Performance Store', file: 'test-performance-store-temporal.js', category: 'Functional' },
  { name: 'Suite 3: Deterministic Performance & Risk-Adjusted Metrics', file: 'test-performance-measurement-metrics.js', category: 'Functional' },
  { name: 'Suite 4: Factor Exposure & Multi-Factor Attribution', file: 'test-performance-factor-exposure.js', category: 'Functional' },
  { name: 'Suite 5: Market & Asset Timing Skill', file: 'test-performance-timing-skill.js', category: 'Functional' },
  { name: 'Suite 6: Security Selection & Sector Allocation Skill', file: 'test-performance-selection-allocation-skill.js', category: 'Functional' },
  { name: 'Suite 7: Process Quality & Mandate Discipline', file: 'test-performance-process-quality.js', category: 'Functional' },
  { name: 'Suite 8: Performance Persistence & Regime Conditionality', file: 'test-performance-persistence-regime.js', category: 'Functional' },
  { name: 'Suite 9: Luck, Bootstrap Significance & Multiple Testing Control', file: 'test-performance-luck-multiple-testing.js', category: 'Functional' },
  { name: 'Suite 10: Capacity, Scalability & Liquidity Drag', file: 'test-performance-capacity-scalability.js', category: 'Functional' },
  { name: 'Suite 11: Manager & Strategy Skill Scorecard', file: 'test-performance-manager-scorecard.js', category: 'Functional' },
  { name: 'Suite 12: Sealed Package & Explanation DAG', file: 'test-performance-package-explanation-dag.js', category: 'Functional' },
  { name: 'Suite 13: Golden Traces A–T (20 Institutional Archetypes)', file: 'test-performance-golden-traces.js', category: 'Golden' },
  { name: 'Suite 14: Hostile Adversarial & Boundary Tests (>= 500 assertions)', file: 'test-performance-hostile.js', category: 'Hostile' },
  { name: 'Suite 15: Mutations, Temporal, Concurrency & HTTP Suite', file: 'test-performance-mutations-concurrency-http.js', category: 'Mutations_Temporal_Concurrency_HTTP' }
];

console.log('================================================================');
console.log('INVESTMENTAI — PHASE 29 PERFORMANCE MEASUREMENT & MANAGER SKILL');
console.log('================================================================\n');
...
for (const suite of testSuites) {
  ...
  if (suite.file === 'test-performance-mutations-concurrency-http.js') {
...
console.log('\n================================================================');
console.log('PHASE 29 EXPLICIT CATEGORY ACCOUNTING TABLE');
console.log('================================================================');
...
console.log('\n================================================================');
console.log('PHASE 29 SUMMARY:');
console.log(`Phase 29 assertion total = ${calculatedAssertionTotal}`);
console.log(`Phase 29 operation total = ${totalOperations}`);
console.log(`Phase 29 suites = ${testSuites.length}`);
console.log(`Failures = ${totalFailed}`);
console.log(`Hostile Assertions >= 500: ${hostileAssertions >= 500 ? 'PROVEN (' + hostileAssertions + ')' : 'FAIL'}`);
console.log(`Mutation Assertions >= 150: ${mutationAssertions >= 150 ? 'PROVEN (' + mutationAssertions + ')' : 'FAIL'}`);
console.log(`Temporal Assertions >= 90: ${temporalAssertions >= 90 ? 'PROVEN (' + temporalAssertions + ')' : 'FAIL'}`);
...
if (totalFailed > 0 || mutationAssertions < 150 || temporalAssertions < 90 || concurrencyOperations < 50 || hostileAssertions < 500 || httpAssertions < 15 || rbacAssertions < 15 || tenantIsolationAssertions < 15) {
  process.exit(1);
}
